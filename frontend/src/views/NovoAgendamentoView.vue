        ...
        <form @submit.prevent="agendar">
          ...
          <div v-if="auth.podeGerenciar" class="form-section">
            ...
            <div class="form-group" style="position:relative">
              ...
              <div v-if="mostrarDropdown && buscaPaciente.length >= 2 && !pacientes.length" class="paciente-dropdown">
                <div class="paciente-vazio text-sm text-muted">Nenhum paciente encontrado</div>
              </div>
            </div>
            <div v-if="pacienteSelecionado" class="paciente-selecionado">
              <span>👤 {{ nomePacienteSelecionado }}</span>
              <button type="button" class="limpar-paciente" @click="limparPaciente">✕ Limpar</button>
            </div>
          ...
          <div class="form-section" :class="{ 'mt-3': auth.podeGerenciar }">
            ...
            <div class="form-row">
              <div class="form-group">
                ...
                <span v-if="erros.especialidade" class="form-error">{{ erros.especialidade }}</span>
              </div>
              <div class="form-group">
            ...
            <div class="form-row">
              ...
              <div class="form-group">
                ...
                <div v-if="buscandoHorarios" class="text-sm text-muted">🔄 Verificando...</div>
                <div v-else-if="horariosDisponiveis.length" class="horarios-grid">
                  <button type="button" v-for="h in horariosDisponiveis" :key="h"
                ...
                <span v-if="erros.horario" class="form-error">{{ erros.horario }}</span>
              </div>
            </div>

            <div class="form-group">
              <label class="form-label">Observações</label>
              <textarea v-model="form.observacoes" class="form-input" rows="3"
                placeholder="Sintomas, exames a trazer, etc."></textarea>
            </div>
          </div>
            ...
            <div class="form-row">
              <div class="form-group">
                ...
                <div style="position:relative">
                  ...
                  <span v-if="buscandoCep" class="spinner spinner-dark"
                    style="position:absolute;right:12px;top:50%;transform:translateY(-50%)"></span>
                </div>
              </div>
              <div class="form-group">
          ...
          <div class="form-actions mt-3">
            <RouterLink to="/dashboard" class="btn btn-secondary">Cancelar</RouterLink>
  ...
  try {
    await api.post('/agendamentos', {
      ...
      enderecoConsulta: form.logradouro ? {
        cep: form.cep, logradouro: form.logradouro,
...
.horario-btn {
  padding: 7px 13px; border: 1.5px solid var(--borda); border-radius: var(--radius-sm);
  background: var(--branco); cursor: pointer; font-size: 13px; font-family: var(--fonte-body);
  transition: all 0.15s; color: var(--texto);
}
.horario-btn:hover { border-color: var(--verde-claro); background: var(--verde-pale); }
.horario-btn.selecionado { background: var(--verde); color: #fff; border-color: var(--verde); }
.clima-card { background: var(--creme-escuro); border-radius: var(--radius-sm); padding: 14px; border: 1px solid var(--borda); }
.clima-header { display: flex; align-items: center; gap: 10px; }
.clima-icone { width: 48px; height: 48px; }
.clima-desc { text-transform: capitalize; font-weight: 500; }
