<template>
  <div>
    <div class="page-header">
      ...
      <RouterLink to="/agendar" class="btn btn-primary btn-sm"
        >+ Nova Consulta</RouterLink
      >
    </div>
    ...
    <div
      v-else-if="agendamentosExibidos.length === 0"
    ...
    <div v-else-if="!auth.podeGerenciar" class="lista mt-3">
      <div
        v-for="ag in agendamentosExibidos"
        :key="ag._id"
        class="ag-item card"
      >
        <div class="ag-data-bloco">
          ...
          <div class="ag-ano text-sm text-muted">
            {{ formatarAno(ag.data) }}
          </div>
        </div>
        <div class="ag-corpo">
          ...
          <div class="ag-detalhes text-sm text-muted mt-1">
            ...
            <span v-if="ag.enderecoConsulta?.cidade"
              >📍 {{ ag.enderecoConsulta.cidade }}</span
            >
            <span v-if="ag.previsaoClimatica?.chuva" class="clima-badge"
              >🌧️ Chuva prevista</span
            >
          </div>
          <div v-if="ag.observacoes" class="ag-obs text-sm mt-1">
            {{ ag.observacoes }}
          </div>
        </div>
        <div class="ag-acoes">
          ...
          <span
            v-else-if="statusFinal(ag.status)"
            class="status-final text-sm text-muted"
          >
    ...
    <div v-else class="card mt-3 table-wrapper">
          ...
          <tr v-for="ag in agendamentosExibidos" :key="ag._id">
            <td>
              ...
              <div class="text-sm text-muted">
                {{ ag.paciente?.telefone || ag.paciente?.email }}
              </div>
            </td>
            <td>
            ...
            <td>
              ...
              <select
                ...
                :value="ag.status"
                @change="atualizarStatus(ag._id, $event.target.value)"
              >
                <option value="agendado">Agendado</option>
                <option value="confirmado">Confirmado</option>
                <option value="realizado">Realizado</option>
      ...
      <div class="paginacao mt-2">
        ...
        <span class="text-sm text-muted"
          >Pág. {{ pagina }} de {{ totalPaginas }}</span
        >
        <button
          class="btn btn-sm btn-secondary"
...
const formatarDia = (d) =>
  new Date(d).toLocaleDateString("pt-BR", { day: "2-digit", timeZone: "UTC" });
const formatarMes = (d) =>
  new Date(d)
...
onMounted(async () => {
  if (auth.podeGerenciar) {
    ...
    try {
      ...
      agendamentos.value = (data.agendamentos || []).sort(
